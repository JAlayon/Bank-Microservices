package com.alayon.msbank.gateway.filters;
...
public class JwtAuthenticationFilter extends AbstractGatewayFilterFactory<JwtAuthenticationFilter.Config> {
    ...
    public GatewayFilter apply(final Config config) {
        ...
        return (exchange, chain) -> {
            ...
            try {
                final Key hmacKey = new SecretKeySpec(secret.getBytes(), SignatureAlgorithm.HS256.getJcaName());
                final Jws<Claims> jwt = Jwts.parserBuilder().setSigningKey(hmacKey).build().parseClaimsJws(parts[1]);
            } catch (final Exception e) {
